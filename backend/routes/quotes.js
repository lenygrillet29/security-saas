...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const countRow = await client.query(
      `SELECT COUNT(*) as c FROM quotes WHERE created_at >= $1`, [`${year}-01-01`]
    );
    ...
    const qRes = await client.query(
      ...
      [client_id, site_id || null, quote_number, title,
...
router.put('/:id', async (req, res) => {
  const client = await db.pool.connect();
  try {
    const existing = await db.get('SELECT id FROM quotes WHERE id = ?', [req.params.id]);
    ...
    const total_ht = calcTotal(lines);
    await client.query(
      `UPDATE quotes SET client_id=$1, site_id=$2, title=$3, valid_until=$4, hourly_rate_day=$5, hourly_rate_night=$6,
       hourly_rate_sunday=$7, status=$8, notes=$9, total_ht=$10, tva_rate=$11 WHERE id=$12`,
